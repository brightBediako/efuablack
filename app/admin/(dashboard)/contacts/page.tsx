...
export default async function AdminContactsPage() {
  await connectDB();
  const rows = (await Contact.find().sort({ createdAt: -1 }).limit(500).lean()) as (ContactDoc & {
    _id: unknown;
  })[];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-headline text-3xl text-primary">Contacts</h1>
        <p className="mt-1 font-body text-sm text-on-surface-variant">Latest 500 messages.</p>
      </div>

      {rows.length === 0 ? (
        <p className="rounded-xl border border-outline-variant/50 bg-surface-container-low p-8 text-on-surface-variant">No contact messages yet.</p>
      ) : (
